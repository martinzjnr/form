class RegistrationForm {
  ...
  updateProgress() {
    ...
    if (progress === 0) {
      ...
      this.progressText.textContent = `${Math.round(progress)}% completed - Keep going!`
    } else if (progress < 100) {
      this.progressText.textContent = `${Math.round(progress)}% completed - Almost there!`
    } else {
  ...
  checkUsernameAvailability() {
    ...
    if (username.length >= 3) {
      ...
      setTimeout(() => {
        ...
        const takenUsernames = ["admin", "user", "test", "crypto", "trader", "bitcoin", "ethereum"]
  ...
  showFieldError(fieldName, message, isError) {
    const field = this.fields[fieldName]
    const errorElement = field.closest(".form-group, .terms-section").querySelector(".error-message")
  ...
  showNotification(message, type) {
    ...
    notification.style.cssText = `
            ...
            left: 16px;
            right: 16px;
            background: ${type === "success" ? "linear-gradient(135deg, #4caf50, #8bc34a)" : "linear-gradient(135deg, #e74c3c, #ff5722)"};
            color: white;
            padding: 20px 24px;
            border-radius: 20px;
